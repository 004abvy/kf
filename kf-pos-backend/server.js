...
// 2️⃣ MIDDLEWARE
app.use(cors({
    origin: "https://kf-sigma.vercel.app",
...
app.post("/api/auth/login", async (req, res) => {
  try {
    ...
    const [staff] = await pool.query(
      `SELECT s.*, r.role_name FROM Staff s JOIN Roles r ON s.role_id = r.role_id WHERE s.email = ?`,
      [email]
    );
    if (staff.length === 0)
...
app.get("/api/categories", async (req, res) => {
  try {
    ...
    res.status(500).json({ message: "Error fetching categories", error: err?.message || String(err) });
  }
});
...
app.get("/api/items/:categoryId", async (req, res) => {
  try {
    ...
    res.status(500).json({ message: "Error fetching items", error: err?.message || String(err) });
  }
});
...
app.get("/api/modifiers", async (req, res) => {
  try {
    ...
    res.status(500).json({ message: "Error fetching modifiers", error: error?.message || String(error) });
  }
});

// ── FEATURED ITEMS ── ✅ NEW ROUTE
app.get("/api/featured", async (req, res) => {
  try {
    ...
    res.status(500).json({ message: "Error fetching featured items", error: err?.message || String(err) });
  }
});
...
app.post("/api/checkout", async (req, res) => {
  ...
  try {
    ...
    const [orderResult] = await connection.query(
      `INSERT INTO Orders (order_number, staff_id, order_type, total_amount, order_status, customer_phone, delivery_address) VALUES (?, ?, ?, ?, ?, ?, ?)`,
      [orderNumber, staffId, orderType, total, "Pending", customerPhone || null, deliveryAddress || null]
    );
    const orderId = orderResult.insertId;
    for (const item of items) {
      const modString = item.modifiers?.length > 0 ? item.modifiers.map(m => m.name).join(", ") : null;
      const isStandalone = String(item.variation_id).startsWith("mod-var-");
      await connection.query(
        `INSERT INTO OrderItems (order_id, variation_id, quantity, unit_price, subtotal, modifiers, standalone_name) VALUES (?, ?, ?, ?, ?, ?, ?)`,
        [orderId, isStandalone ? null : item.variation_id, item.quantity, item.price, item.price * item.quantity, modString, isStandalone ? item.item_name : null]
      );
    }
    ...
    await connection.query(
      `INSERT INTO Payments (order_id, payment_method, amount_paid, payment_status, transaction_id) VALUES (?, ?, ?, ?, ?)`,
...
app.get("/api/staff/orders", async (req, res) => {
  try {
    const [orders] = await pool.query(`
      ...
      WHERE o.order_status != 'Completed' AND o.order_status != 'Cancelled' AND o.order_status != 'Rejected'
      ORDER BY o.created_at ASC`);
    for (let order of orders) {
      const [items] = await pool.query(`
        SELECT oi.quantity, COALESCE(m.name, oi.standalone_name) as item_name, CONCAT_WS(' | ', v.size_name, oi.modifiers) as size_name
        FROM OrderItems oi LEFT JOIN ItemVariations v ON oi.variation_id = v.variation_id
        LEFT JOIN MenuItems m ON v.item_id = m.item_id WHERE oi.order_id = ?`, [order.order_id]);
      order.items = items;
    }
    res.json(orders);
  } catch (error) {
...
app.post("/api/staff/update-status", async (req, res) => {
  try {
    ...
    const [orderData] = await pool.query(
      "SELECT customer_phone, order_number, total_amount FROM Orders WHERE order_id = ?", [orderId]
    );
    ...
    const [itemsData] = await pool.query(`
      SELECT oi.quantity, COALESCE(m.name, oi.standalone_name) as item_name, CONCAT_WS(' | ', v.size_name, oi.modifiers) as size_name
      FROM OrderItems oi LEFT JOIN ItemVariations v ON oi.variation_id = v.variation_id
      LEFT JOIN MenuItems m ON v.item_id = m.item_id WHERE oi.order_id = ?`, [orderId]);
    const itemsList = itemsData.map(item => {
      const details = item.size_name?.trim() ? ` (${item.size_name})` : "";
      return `▫️ ${item.quantity}x ${item.item_name}${details}`;
    }).join("\n");
    await pool.query("UPDATE Orders SET order_status = ?, rejection_reason = ? WHERE order_id = ?",
      [newStatus, rejectionReason || null, orderId]);
    if (newPaymentStatus)
      await pool.query("UPDATE Payments SET payment_status = ? WHERE order_id = ?", [newPaymentStatus, orderId]);
    ...
    if (phone && phone.length >= 10) {
      ...
      if (msg) {
        let formattedPhone = phone.startsWith("0") ? "+92" + phone.substring(1) : phone;
        if (twilioClient) {
          try {
            await twilioClient.messages.create({ body: msg, from: twilioWhatsAppNumber, to: `whatsapp:${formattedPhone}` });
            console.log(`✅ WHATSAPP SENT TO ${formattedPhone}`);
          } catch (smsErr) {
            console.error("❌ WhatsApp Failed:", smsErr.message);
          }
        } else {
...
app.get("/api/admin/stats", async (req, res) => {
  try {
    ...
    const [popular] = await pool.query(`
      ...
      GROUP BY COALESCE(m.name, oi.standalone_name) ORDER BY total_sold DESC LIMIT 5`);
    ...
    const revenueTrend = revenueTrendRaw.map(row => ({
      date: new Date(row.date).toLocaleDateString("en-US", { month: "short", day: "2-digit" }),
      revenue: row.revenue
    }));
    ...
    res.json({ revenue: revenue[0].daily_total || 0, orderCount: counts[0].total_orders, popular, revenueTrend, paymentStats });
  } catch (error) {
...
app.get("/api/customer/history/:phone", async (req, res) => {
  try {
    const [orders] = await pool.query(`
      ...
      FROM Orders WHERE customer_phone = ? ORDER BY created_at DESC LIMIT 10`, [req.params.phone]);
    for (let order of orders) {
      const [items] = await pool.query(`
        SELECT oi.quantity, COALESCE(m.name, oi.standalone_name) as item_name, oi.variation_id, m.image_url, oi.unit_price as price,
...
server.listen(PORT, () => { console.log(`🚀 KF Backend running on port ${PORT}`); });
server.on("error", (err) => {
